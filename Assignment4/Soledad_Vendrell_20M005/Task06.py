...
# TO DO
classes = [
  ontology.Person, ontology.Professor, ontology.FullProfessor,
  ontology.AssociateProfessor, ontology.InterimAssociateProfessor
]
for cls in classes:
    local = str(cls).split('#')[-1]
    g.add((cls, RDF.type, RDFS.Class))
    g.set((cls, RDFS.label, Literal(local, datatype=XSD.string)))
...
g.add((person.Oscar, vcard_Given,  Literal("Óscar", datatype=XSD.string)))
g.add((person.Oscar, vcard_Family, Literal("Corcho García", datatype=XSD.string)))
g.add((person.Oscar, foaf_email,   Literal("ocorcho@fi.upm.es", datatype=XSD.string)))
