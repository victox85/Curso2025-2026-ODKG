...
Original file is located at
    https://colab.research.google.com/drive/1oDulXmgobNOS2KUoqNkcYX8qfZKlsniX
...
def add_subclasses(cls):
    for subclass in g.subjects(RDFS.subClassOf, cls):
        if subclass not in classes_to_check:
            classes_to_check.add(subclass)
            add_subclasses(subclass)

add_subclasses(ns.Person)
...
WHERE {
    {
      ?person :hasColleague ?collegue .
      ?collegue :ownsPet ?dog.
    }
    UNION
    {
      ?person :hasColleague ?collegue1 .
      ?collegue1 :hasColleague ?collegue2 .
      ?collegue2 :ownsPet ?dog.
    }
    ?person rdfs:label ?name
  }
